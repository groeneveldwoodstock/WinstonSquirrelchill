...
class StickHeroGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let StackMaxWidth:CGFloat = 350.0
    let StackMinWidth:CGFloat = 150.0
    ...
    fileprivate func checkTouchMidStack() {
        ...
        if ((stick.position.x + self.stickHeight) >= newPoint.x - 20  && (stick.position.x + self.stickHeight) <= newPoint.x + 20) {
            loadPerfect()
...
private extension StickHeroGameScene {
    ...
    func loadHighScore() {
        let highScoreBand = SKLabelNode(fontNamed: "Arial")
        highScoreBand.name = StickHeroGameSceneChildName.ScoreName.rawValue
        highScoreBand.text = "High Score: " + String(UserDefaults.standard.integer(forKey: StoreScoreName))
        highScoreBand.position = CGPoint(x: 0, y: DefinedScreenHeight / 2 - 300)
        highScoreBand.fontColor = SKColor.white
        highScoreBand.fontSize = 60
        highScoreBand.zPosition = StickHeroGameSceneZposition.scoreZposition.rawValue
        highScoreBand.horizontalAlignmentMode = .center
        
        addChild(highScoreBand)
    }
    func loadLabel() {
        let labelBand = SKLabelNode(fontNamed: "Zapfino")
        labelBand.name = StickHeroGameSceneChildName.ScoreName.rawValue
        labelBand.text = "Winston Squirrelchill"
        labelBand.position = CGPoint(x: 0, y: DefinedScreenHeight / 2 - 500)
        labelBand.fontColor = SKColor.white
        labelBand.fontSize = 60
        labelBand.zPosition = StickHeroGameSceneZposition.scoreZposition.rawValue
        labelBand.horizontalAlignmentMode = .center
        
        addChild(labelBand)
    }
    func loadHighScoreBackground() {
        let highBack = SKShapeNode(rect: CGRect(x: 0-200, y: 1024-300-30, width: 400, height: 100), cornerRadius: 20)
        highBack.zPosition = StickHeroGameSceneZposition.scoreBackgroundZposition.rawValue
        highBack.fillColor = SKColor.black.withAlphaComponent(0.3)
        highBack.strokeColor = SKColor.black.withAlphaComponent(0.3)
        addChild(highBack)
    }
    ...
    func loadTip() {
        ...
        tip.text = "Touch the screen to make the bridge longer"
